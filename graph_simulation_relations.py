...
for path in tqdm(os.listdir(directory)):
    ...
    if os.path.isfile(full_path) and 'stone' in full_path:
        ...
        for i, n in enumerate(ns):
            ...
            proportion_above_stat_s = HeatMapAnalysis(non_weighted_heat_map_r_3).above_treshold_heat_proportion(100 * max_noise_heat_s)
            proportion_above_stat_l = HeatMapAnalysis(non_weighted_heat_map_r_max).above_treshold_heat_proportion(100 * max_noise_heat_l)
...
ax1.scatter(ks, k_props_100mnh_s, color="r", s=2, label="$TNA_s$")
ax1.scatter(ks, k_props_100mnh_l, color="g", s=2, label="$TNA_l$")
plt.xlabel('$k$')
...
ax1.scatter(ns, n_props_100mnh_s, color="r", s=2, label="$TNA_s$")
ax1.scatter(ns, n_props_100mnh_l, color="g", s=2, label="$TNA_l$")
plt.xlabel('$n$')
